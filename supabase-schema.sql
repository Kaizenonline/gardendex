...
begin
  ...
  do update set
    ...
    daily_scan_reset_at = coalesce(credits.daily_scan_reset_at, current_date),
    updated_at = now();
...
begin
  ...
  if not found then
    -- Fix #6: trigger may have failed at signup — auto-create the row with
    -- the default 10 free credits rather than permanently blocking the user.
    insert into credits (user_id, balance, lifetime_used)
    ...
    if not found then
      return false; -- truly missing, give up
    end if;
  end if;
...
-- ── User Data (beds, achievements, streak, statuses) ─────────────────────────
-- Single row per user storing all non-plant state as JSONB
